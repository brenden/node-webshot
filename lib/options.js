 ...
 */
exports.bindSerializable = function(fn, context) {
  if (toString.call(context) !== '[object Object]') {
    throw new Error('Function context ' + context + ' is not a string');
  }

  var signature = fn.slice(0, fn.indexOf(')') + 1)
  var body = fn.slice(fn.indexOf('{') + 1, fn.lastIndexOf('}'));
  var bound = '(function() {' + body + '}).apply('
    + JSON.stringify(context) + ');';

  return eval('(' + signature + '{ return ' + bound + '})');
}
