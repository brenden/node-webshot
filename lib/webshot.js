...
// Set up the option defaults as key/value pairs
var defaults = {};

Object.keys(optionLists).forEach(function(k) {
  optionLists[k].forEach(function(opt) {
...
function spawnPhantom(site, path, streaming, options, cb) {
  ...
  optionLists.phantomPreload
    .concat(optionLists.phantomPostload).forEach(function(opt) {
      filteredOptions[opt.name] = options[opt.name];
  });
