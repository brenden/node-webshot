  ...
  if (!streaming) {
    phantomProc.stderr.on('data', function(data) {
      if (options.errorIfJSException) {
        var err = JSON.parse(data.toString());
        calledCallback = true;
    ...
    phantomProc.stdout.on('data', function(data) {
      ...
      if (cb && !callbackCalled) {
        chunks.push(new Buffer(''+data, 'base64'));
      } else {
    ...
    phantomProc.stderr.on('data', function(data) {
      var err = JSON.parse(data.toString());
      var emitJsError = options.errorIfJSException && err.isJsError;
      var emitNon200Error = options.errorIfStatusIsNot200 && err.isNon200Error;
      if (emitJsError || emitNon200Error) {
    ...
    phantomProc.on('exit', function() {
      if (cb && !callbackCalled) {
        cb(null, s);
        s.emit('data', Buffer.concat(chunks));
      }
      if (!streamEnded) {
        s.emit('end');
      }
