...
function spawnPhantom(site, path, streaming, options, cb) {
  ...
  filteredOptions.exportConsoleOutput = (options.exportConsoleOutput || (options.onConsoleMessage && typeof options.onConsoleMessage === 'function'));
  ...
  if (!streaming) {
    ...
    if (options.onConsoleMessage && typeof options.onConsoleMessage === 'function') {
      phantomProc.stdout.on('data', function(data) {
        var lines = data.toString().split(/(\r?\n)/g);
        lines.map(function(text){
          if(text.substr(0,10) === '[console] ') {
            options.onConsoleMessage(text.substring(10))
          }
        });
      });
    }
    else if (options.exportConsoleOutput) {
      phantomProc.stdout.on('data', function(data) {
        var lines = data.toString().split(/(\r?\n)/g);
        lines.map(function(text){
          if(text.substr(0,10) === '[console] ') {
            process.stdout.write(text.substring(10)+"\n")
          }
        })
      });
    }
