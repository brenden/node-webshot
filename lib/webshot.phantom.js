...
optUtils.phantomCallback.forEach(function(cbName) {
  ...
  if (cbName === 'onConsoleMessage' && cb) {
    page[cbName] = buildFn(cb.fn, cb.context);
    return;
  }
 ...
 */
function buildFn(fn, context) {
  return function() {
    eval('(' + fn + ')').apply(context, arguments);
  };
}

/*
 * Bind the function `fn` to the context `context` to the page in a serializable manner.
 * A tiny bit of a hack.
 *
 * @param (String) fn
 * @param (Object) context
