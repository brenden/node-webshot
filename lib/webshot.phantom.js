  ...
  window.setTimeout(function () {
    ...
    if (options.captureSelector) {
      ...
      page.clipRect = page.evaluate(function(selector, zoomFactor) {
        try {
          var selectorElem = document.querySelector(selector);
          var selectorClipRect = selectorElem.getBoundingClientRect();

          return {
              top: selectorClipRect.top * zoomFactor
            , left: selectorClipRect.left * zoomFactor
            , width: Math.max(selectorClipRect.width, selectorElem.scrollWidth) * zoomFactor
            , height: Math.max(selectorClipRect.height, selectorElem.scrollHeight) * zoomFactor
          };
        } catch (e) {
