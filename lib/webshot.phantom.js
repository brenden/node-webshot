...
optUtils.phantomCallback.forEach(function(callbackName) {
  var cb = options[callbackName];

  if (cb) {
    page[callbackName] = optUtils.bindSerializable(cb.fn, cb.context);
  }
})
...
var whenLoadFinished = function(status) {
  ...
  window.setTimeout(function () {
    ...
    page.evaluate(
      optUtils.bindSerializable(options.script.fn, options.script.context));
